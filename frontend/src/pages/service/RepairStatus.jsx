...
import { Check, Clipboard, Wrench, Search, ShieldCheck, Car, RefreshCw } from 'lucide-react';
...
const RepairStatus = () => {
  ...
  const fetchJobCards = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        // Retain selection if activeJob was already set, otherwise default to first card
        if (cards.length > 0) {
          if (activeJob) {
            const currentSelected = cards.find(c => c.id === activeJob.id);
            setActiveJob(currentSelected || cards[0]);
          } else {
            setActiveJob(cards[0]);
          }
        } else {
          setActiveJob(null);
        }
      ...
      setError('Failed to fetch active repair records.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const currentStageIndex = activeJob ? timelineStages.findIndex(s => s.id === activeJob.status) : 0;

  return (
    <div className="space-y-6">
      <div className="mb-6 flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-display font-bold text-white mb-2">Repair Status Tracker</h1>
          <p className="text-gray-400">Track the live progress of workshop repairs in real-time.</p>
        </div>
        <button 
      ...
      {loading && jobCards.length === 0 ? (
        <div className="text-center py-12 text-gray-400">Loading active repairs...</div>
      ) : error ? (
        <div className="text-center py-12 text-red-400">{error}</div>
      ) : jobCards.length === 0 ? (
        <div className="bg-charcoal-800/40 border border-white/5 p-12 rounded-3xl text-center text-gray-500">
          No vehicles are currently undergoing repairs in the workshop.
        </div>
      ) : (
          ...
          <div className="lg:col-span-1 space-y-4">
            <Card>
              <div className="p-4 border-b border-white/5">
                <h3 className="font-display font-semibold text-white">Active Workshop Cards</h3>
              </div>
                ...
                >
                  <Card className="h-full">
                    <CardContent className="p-8">
                      <div className="mb-8">
                        <h2 className="text-2xl font-display font-bold text-white mb-2">Job Card Details: {activeJob.id}</h2>
                        <div className="flex flex-wrap gap-4 text-sm text-gray-400">
                          <span><strong>Customer:</strong> {activeJob.customerName}</span>
                          <span><strong>Vehicle:</strong> {activeJob.carMake}</span>
                          <span><strong>Est. Completion:</strong> {activeJob.expectedCompletion || 'TBD'}</span>
                        </div>
                      </div>
                      ...
                      <div className="mt-16 bg-white/5 rounded-xl p-6 border border-white/5">
                        <h4 className="font-semibold text-white mb-2">Reported Work Scope</h4>
